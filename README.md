...
- **⏱️ Update Cadence (what "live" actually means):** `agy` re-runs the statusline command on store events, coalesced by a 300 ms debounce — so while the agent is streaming or you are typing, it re-renders roughly 3x/second. There is no timer and no configurable refresh interval (`statusLine` accepts only `type`, `command`, `padding`, `enabled`, `stack_with_default`), so **at full idle nothing re-runs it** and the line holds its last value until the next keystroke or agent event. Within that ceiling: the reset countdown is recomputed every render, and the quota percentage is re-measured in the background every `AGY_STATUSLINE_SYNC_SECS` seconds (default 20, halved when below 15% remaining). Set that lower for fresher numbers at the cost of one short background process per interval.
